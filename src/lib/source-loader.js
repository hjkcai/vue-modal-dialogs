// This is a fork from https://github.com/QingWei-Li/vue-markdown-loader
  ...
  if (typeof opts.render === 'function') {
    ...
    !preventExtract &&
      parser.core.ruler.push('extract_script_or_style', function replace (
        ...
        let tagRegex = new RegExp('<(script|style)(?:[^<]|<)+</\\1>', 'g')
        let newTokens = []
        state.tokens
          .filter(token => token.type === 'fence' && token.info === 'html')
          .forEach(token => {
            let tokens = (token.content.match(tagRegex) || []).map(content => {
